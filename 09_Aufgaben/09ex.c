...
PileOfPancakes *pure_fruit_ptr() {
    PileOfPancakes p = { .layer = Fruit, .further_layers = NULL };
    PileOfPancakes *pp = (PileOfPancakes*) malloc(sizeof(p));
    *pp = p;
    return pp;
}
...
PileOfPancakes *fruit_crepe_ptr() {
    PileOfPancakes p1 = { .layer = Fruit, .further_layers = NULL };
    PileOfPancakes *pp1 = (PileOfPancakes*) malloc(sizeof(PileOfPancakes));
    *pp1 = p1;
    PileOfPancakes p0 = { .layer = ActualPancake, .further_layers = pp1 };
    PileOfPancakes *pp0 = (PileOfPancakes*) malloc(sizeof(PileOfPancakes));
    *pp0 = p0;
    return pp0;
    //NOTE: valgrind beschwert sich beim unteren code, warum??

    // PileOfPancakes *pancake = (PileOfPancakes*) malloc(sizeof(PileOfPancakes) * 2);
    // PileOfPancakes p2 = { .layer = Fruit, .further_layers = NULL};
    // *pancake = p2;
    // printf("%d\n", sizeof(pancake));
    // PileOfPancakes p1 = { .layer = ActualPancake, .further_layers = pancake};
    // *(pancake + 1) = p1;
    // return &pancake[1];
}
...
PileOfPancakes mille_crepes(uint32_t n) {
    ...
    for (int i=1; i<n; i++) {
      PileOfPancakes next = { .layer = ActualPancake, .further_layers = NULL };
      PileOfPancakes *nextp = malloc(sizeof(p));
      *nextp = next;
      cur->further_layers = nextp;
      cur = cur->further_layers;
    }
    return p;
}
...
uint16_t *create_dynamic_array(size_t x) {
    ...
    // compiler passt das int i an, sodass ich nicht uint_t i schreiben muss
    for (int i=1; i<=x; i++) *(arr + i-1) = i*i;
    return arr;
}
