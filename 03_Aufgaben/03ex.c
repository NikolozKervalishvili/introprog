...
int sum_of_divisibles(int n, int m) {
    int result = 0;
    for (int i=1; n>0; i++) {
        n -= is_multiple_of(i, m);
        result += i*is_multiple_of(i, m);
    }
    return result;
}
...
int sum_of_doubly_divisibles(int n, int l, int m) {
    int result = 0;
    for (int i=1; n>0; i++) {
        n -= is_multiple_of(i, m)*is_multiple_of(i, l);
        result += i*is_multiple_of(i, m)*is_multiple_of(i, l);
    }
    return result;
}
...
Canvas paint_it_black(Canvas c) {
    for (int i=0; i<(canvas_width(c)*canvas_height(c)); i++) 
        c = canvas_set_black(c, i%canvas_width(c), i/canvas_width(c));
    return c;
}
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    for (int i=0; i<canvas_height(c); i++) {
      for (int j=0; j<canvas_width(c); j++) {
        if (j < x+width && j >= x && i > y-height && i <= y) {
            c = canvas_set_black(c, j, i);
        }
      }
    }
...
Canvas draw_rectangle_via_corners(Canvas c, int x0, int y0, int x1, int y1) {
    // for (int i=0; i<canvas_height(c); i++) {
    //   for (int j=0; j<canvas_width(c); j++) {
    //     if (j <= x1 && j >= x0 && i >= y1 && i <= y0) {
    //         c = canvas_set_black(c, j, i);
    //     }
    //   }
    // }
    c = draw_rectangle(c, x0, y0, x1-x0+1, y0-y1+1);
    return c;
}
